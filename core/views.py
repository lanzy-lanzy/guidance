...
def simple_interview_form(request, student_id):
    student = get_object_or_404(Student, id=student_id)
    counselor = get_object_or_404(Counselor, user=request.user)
    
    # Get the latest approved appointment for this student and counselor
    ...
    # Only allow interview if there's an approved appointment or if the counselor is accessing from the student list
    if not appointment:
        messages.warning(request, "No approved appointment found for this student. The interview will be created without an appointment reference.")
    
    if request.method == 'POST':
        # Create new session and interview
        ...
        # If there's an appointment, link it to the session and mark it as completed
        if appointment:
...
def appointment_list(request):
    # Get all appointments for the counselor
    appointments = Appointment.objects.filter(counselor=request.user.counselor).order_by('-date', '-time')
...
def session_history(request):
    ...
    # Get all sessions for the logged-in counselor
    sessions = GuidanceSession.objects.filter(counselor=request.user.counselor).order_by('-date')
    ...
    return render(request, 'counselor/session_history.html', context)
...
def update_student_profile(request):
    if request.method == 'POST' and request.user.role == 'student':
        ...
        if 'profile_picture' in request.FILES:
            student.user.profile_picture = request.FILES['profile_picture']
            student.user.save()
...
def print_interview(request, interview_id):
    ...
    # Ensure the counselor can only access their own interviews
    if request.user.counselor_profile != interview.counselor:
